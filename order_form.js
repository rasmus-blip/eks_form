...
export function setUpOrderForm() {
  ...
  //SUBMIT ORDER BTNS
  document
    .querySelector("#your_order .submit")
    .addEventListener("click", submitReqFromYourOrder);
  document
    .querySelector("#checkout button")
    .addEventListener("click", submitReqFromCheckout);
}
...
function setUpSignInField() {
  ...
  createAccBtn.addEventListener("click", () => {
    document.querySelector("#viewport_slide").style.transform =
      "translateY(-50%)";
...
//
//
// BEER-CATALOGUE - FIELDSET
// get the endpoint
async function getBeerTypes() {
  const url = "https://foobarfirefjerdedele.herokuapp.com/beertypes";
  const beers = await getJSON(url, "headersHeroku");
  showBeersCatalogue(beers);
}

// append the beer info
function showBeersCatalogue(beers) {
  const container = document.querySelector("#catalogue_list");
  const beerTemplate = document.querySelector("#beer_catalogue");
  container.innerHTML = "";
  beers.forEach((beer) => {
    let klon = beerTemplate.cloneNode(true).content;
    klon.querySelector(".beer_name").textContent += beer.name;
    klon.querySelector(".beer_image").src = `/images/${beer.label}`;
    klon.querySelector(".beer_category span").textContent +=
      " " + beer.category;
    klon.querySelector(".beer_alc_level span").textContent +=
      " " + beer.alc + "%";
    klon.querySelector(".beer_aroma span").textContent +=
      " " + beer.description.aroma;
    klon.querySelector(".beer_appearance span").textContent +=
      " " + beer.description.appearance;
    klon.querySelector(".beer_flavour span").textContent +=
      " " + beer.description.flavor;
    klon.querySelector(".beer_mouthfeel span").textContent +=
      " " + beer.description.mouthfeel;
    klon.querySelector(".beer_overall_impression span").textContent +=
      " " + beer.description.overallImpression;
    container.appendChild(klon);
  });

  //slide to next fieldset
  const catalogueNextBtn = document.querySelector(
    "#beer_catalogue_container .next"
  );
  catalogueNextBtn.addEventListener("click", (e) => {
    slideFieldset(e.target, "order_form");
  });
}
